...
def build_stylish_iter(diff: dict, depth=0) -> str:
    ...
    for dictionary in diff:
        ...
        if operation == 'same':
            lines.append(line_forming(dictionary, 'value', depth, sign='    '))

        elif operation == 'add':
            lines.append(line_forming(dictionary, 'new', depth, sign='  + '))

        elif operation in ('removed', 'changed'):
            lines.append(line_forming(dictionary, 'old', depth, sign='  - '))
